...
// === Tracking-control-name selectors used by LinkedIn's top card.
// On SPA job navigation LinkedIn re-renders the top card. These anchors are
// guaranteed to belong to the currently-displayed job so they're the most
// reliable source for title/company/logo without shadow-DOM gymnastics.
const TOPCARD_TITLE_ANCHOR_SELECTOR =
  'a[data-tracking-control-name="public_jobs_topcard-title"]';
const TOPCARD_LOGO_ANCHOR_SELECTOR =
  'a[data-tracking-control-name="public_jobs_topcard_logo"]';
const TOPCARD_COMPANY_ANCHOR_SELECTOR =
  'a[data-tracking-control-name="public_jobs_topcard-org-name"]';

/**
 * Returns true when the anchor clearly belongs to the given LinkedIn job id.
 * If the anchor doesn't expose any job id we accept it as a match (the
 * topcard anchors in scope are top-level on the rendered card and therefore
 * implicitly scoped to the currently-displayed job).
 */
function anchorMatchesJobId(anchor: Element, jobId: string): boolean {
  ...
  // No id marker found — accept the anchor; the topcard scopes implicitly.
  return true;
}

function getLinkedInTopcardJobId(): string | null {
  // Prefer the explicit URL job id (currentJobId or /jobs/view/<id>).
  const fromParam = new URL(window.location.href).searchParams.get('currentJobId');
  if (fromParam) return fromParam;
  const viewMatch = window.location.pathname.match(/\/jobs\/view\/(\d+)/);
  if (viewMatch) return viewMatch[1];
...
function extractCompanyName(root: Element): string {
  ...
  // First and most reliable: the dedicated org-name anchor.
  ...
  // Next: the logo anchor's accessible name (also rendered per-job on navigation).
  ...
  // Shadow-walking aria-label search (the legacy code used root.querySelector
  // which can't reach into SDUI shadow roots, so it missed the company name on
  // new job navigation).
...
function extractCompanyLogoUrl(root: Element): string {
  ...
  // Primary: LinkedIn's topcard logo anchor re-renders on every SPA job nav
  // and is scoped to the current job. Walk shadow DOM so SDUI is covered.
  const logoAnchors = queryDeepAll(TOPCARD_LOGO_ANCHOR_SELECTOR, root);
  for (const anchor of logoAnchors) {
    if (topcardJobId && !anchorMatchesJobId(anchor, topcardJobId)) continue;
  ...
  // Secondary: page-wide shadow-walking search for media.licdn.com images.
  ...
  // Broader page-wide fallback for any licdn.com hosted image.
  ...
  for (const img of genericLicdn) {
    ...
    // Skip obvious non-company assets (user avatars, illustrations).
    const alt = (img.alt || '').toLowerCase();
    if (
      alt &&
      !alt.includes('company') &&
      !alt.includes('logo') &&
      !alt.includes('employer')
    ) {
      continue;
    }
    return src;
