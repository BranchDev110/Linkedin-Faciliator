...
export default function JobsPage() {
  ...
  const filteredJobs = useMemo(() => {
    ...
    return jobs.filter((job) =>
      [job.companyName, job.jobTitle, job.linkedInJobId]
        .join(' ')
        .toLowerCase()
        .includes(query),
    );
            ...
            <div className="jobs-list">
              {filteredJobs.map((job) => {
                ...
                const status = application
                  ...
                  : null;
                const hasResume = applicationHasResume(application);
                const isApplied = applicationIsApplied(application);

                return (
                  <button
                    key={job.id}
                    type="button"
                    className={`jobs-list-item${selectedJob?.id === job.id ? ' active' : ''}`}
                  ...
                  >
                    ...
                    {application ? (
                      <div className="jobs-list-item-status">
                        <span className={`badge ${applicationStatusClass(status!)}`}>
                          {isApplied
                            ? 'Applied'
                            : hasResume
                              ? 'Resume generated'
              ...
              {!filteredJobs.length && <p className="jobs-empty">No jobs found.</p>}
            </div>
          </aside>
